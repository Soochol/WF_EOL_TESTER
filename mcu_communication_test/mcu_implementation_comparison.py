...
class MCUImplementationComparator:
    ...
    async def test_original_mcu(self, wait_time: float = 0.1) -> Dict[str, Any]:
        ...
        try:
            ...
            for i, (cmd_name, cmd_func) in enumerate(commands, 1):
                ...
                print(f"⏱️ 소요시간: 실행 {execution_time*1000:.1f}ms + 대기 {wait_time:.1f}s = 총 {total_time:.3f}s")
        ...
        print(f"   성공률: {success_count}/{len(commands)} ({success_count/len(commands)*100:.1f}%)")
    ...
    def analyze_performance_differences(self) -> Dict[str, Any]:
        ...
        # 안정성 비교
        orig_success_rate = len([r for r in self.test_results["original"] if r["success"]]) / max(1, len(self.test_results["original"]))
        fast_success_rate = len([r for r in self.test_results["fast"] if r["success"]]) / max(1, len(self.test_results["fast"]))
