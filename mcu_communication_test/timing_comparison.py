...
class TimingComparator:
        ...
        for dataset_name, dataset in datasets:
            ...
            for result in dataset:
                if result.get("success"):
                    ...
                    print(f"   {command}:")
                    print(
                        f"     실행: {exec_time*1000:.1f}ms, 대기: {wait_time:.1f}s, 총계: {total_cmd_time:.3f}s"
                    )
            ...
            if success_count > 0:
                ...
                print(
                    f"     성공률: {success_count}/{len(dataset)} ({success_count/len(dataset)*100:.1f}%)"
                )
    ...
    def compare_performance(self):
        ...
        if self.simple_results and self.current_no_delays:
            simple_responses = [
                r.get("response_time_ms", 0) for r in self.simple_results if r.get("success")
            ]
            current_executions = [
                r.get("execution_time", 0) * 1000
                for r in self.current_no_delays
                if r.get("success")
            ]
            ...
            if simple_responses and current_executions:
                ...
                print(
                    f"   오버헤드: +{current_avg - simple_avg:.1f}ms ({((current_avg/simple_avg)-1)*100:.1f}%)"
                )

        # 대기시간 효과 분석
        if self.current_with_delays and self.current_no_delays:
            with_delays_time = sum(
                [r.get("total_time", 0) for r in self.current_with_delays if r.get("success")]
            )
            no_delays_time = sum(
                [r.get("total_time", 0) for r in self.current_no_delays if r.get("success")]
            )
            ...
            print(f"\n🔄 대기시간 효과:")
            print(
                f"   3초 대기 포함: {with_delays_time:.3f}s (성공: {with_delays_success}/{len(self.current_with_delays)})"
            )
            print(
                f"   대기시간 없음: {no_delays_time:.3f}s (성공: {no_delays_success}/{len(self.current_no_delays)})"
            )

            if with_delays_success > 0 and no_delays_success > 0:
                time_saved = with_delays_time - no_delays_time
                success_rate_diff = (no_delays_success / len(self.current_no_delays)) - (
                    with_delays_success / len(self.current_with_delays)
                )
    ...
    def identify_bottlenecks(self):
        ...
        if self.simple_results and self.current_no_delays:
            ...
            simple_avg = sum(
                [r.get("response_time_ms", 0) for r in self.simple_results if r.get("success")]
            ) / max(1, len([r for r in self.simple_results if r.get("success")]))
            current_avg = sum(
                [
                    r.get("execution_time", 0) * 1000
                    for r in self.current_no_delays
            ...
            ) / max(1, len([r for r in self.current_no_delays if r.get("success")]))
            ...
            if failed_commands:
                ...
                for cmd in failed_commands:
                    print(
                        f"      - {cmd.get('command', 'Unknown')}: {cmd.get('error', 'Unknown error')}"
                    )
    ...
    def generate_recommendations(self):
        ...
        if self.current_no_delays:
            no_delay_success_rate = len(
                [r for r in self.current_no_delays if r.get("success")]
            ) / len(self.current_no_delays)
        ...
        try:
            with open(filename, "w", encoding="utf-8") as f:
                ...
                if self.current_no_delays:
                    ...
                    f.write(
                        f"- **현재 코드 (대기 없음)**: 평균 {current_avg:.1f}ms 실행, 성공률 {success_rate*100:.1f}%\n"
                    )
                ...
                if self.current_with_delays:
                    total_time = sum(
                        [
                            r.get("total_time", 0)
                            for r in self.current_with_delays
                            if r.get("success")
                        ]
                    )
