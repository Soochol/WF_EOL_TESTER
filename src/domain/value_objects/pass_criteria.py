...
class PassCriteria:
    ...
    spec_points: List[Tuple[float, float, float, float]] = (
    ...
    def is_temperature_within_limits(
    ...
    def is_position_within_tolerance(
    ...
    def is_measurement_stable(
        ...
        tolerance = (
            self.measurement_tolerance * tolerance_factor
        )
        avg = sum(measurements) / len(measurements)

        return all(
            abs(m - avg) <= tolerance for m in measurements
        )
    ...
    def format_temperature(self, temperature: float) -> str:
        ...
        return (
            f"{temperature:.{self.temperature_precision}f}"
        )
    ...
    def to_dict(self) -> Dict[str, Any]:
        ...
        return {
            ...
            "spec_points": [
                list(point) for point in self.spec_points
            ],
        }
    ...
    def from_dict(
        ...
        if "spec_points" in data:
            spec_points = [
                tuple(point)
                for point in data["spec_points"]
            ]

        return cls(
            force_limit_min=data.get(
                "force_limit_min", 0.0
            ),
            force_limit_max=data.get(
                "force_limit_max", 100.0
            ),
            temperature_limit_max=data.get(
                "temperature_limit_max", 80.0
            ),
            temperature_limit_min=data.get(
                "temperature_limit_min", -10.0
            ),
            measurement_tolerance=data.get(
                "measurement_tolerance", 0.001
            ),
            force_precision=data.get("force_precision", 2),
            temperature_precision=data.get(
                "temperature_precision", 1
            ),
            position_tolerance=data.get(
                "position_tolerance", 0.5
            ),
            max_test_duration=data.get(
                "max_test_duration", 300.0
            ),
            min_stabilization_time=data.get(
