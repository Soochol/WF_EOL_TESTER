...
class DUTCommandInfo:
    ...
    def _validate_fields(self) -> None:
        ...
        if not self.serial_number or not self.serial_number.strip():
            raise ValidationException("serial_number", self.serial_number, "Serial number is required")
        ...
        if len(self.model_number.strip()) > 100:
            raise ValidationException("model_number", self.model_number, "Model number too long (max 100 characters)")

        if len(self.serial_number.strip()) > 50:
            raise ValidationException("serial_number", self.serial_number, "Serial number too long (max 50 characters)")

        if len(self.manufacturer.strip()) > 100:
            raise ValidationException("manufacturer", self.manufacturer, "Manufacturer name too long (max 100 characters)")
