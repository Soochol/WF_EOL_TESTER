...
ENCODING = (
    "ascii"  # Standard encoding for industrial protocols
)

# Buffer Settings
READ_BUFFER_SIZE = 1024  # Read buffer size in bytes
MAX_COMMAND_LENGTH = (
    100  # Maximum command length in characters
)
MAX_RESPONSE_LENGTH = (
    512  # Maximum expected response length
)

# Retry and Error Handling
MAX_RETRY_ATTEMPTS = (
    3  # Maximum retry attempts for failed operations
)
RETRY_DELAY = 0.1  # Delay between retry attempts (seconds)
FLUSH_TIMEOUT = (
    0.1  # Timeout for buffer flushing operations
)
...
BS205_TERMINATOR = (
    "\r"  # BS205 LoadCell specific terminator
)
