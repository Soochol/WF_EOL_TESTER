...
class BaseUseCase(ABC):
    ...
    async def execute(self, command: BaseCommand) -> BaseResult:
        """
        ...
        logger.info(f"Starting use case '{self._use_case_name}' with test_id={context.test_id.value}")
        ...
        try:
            ...
            execution_duration = TestDuration(
                context.end_time.value - context.start_time.value
            )
    ...
    @abstractmethod
    async def _execute_implementation(self, command: BaseCommand, context: ExecutionContext) -> BaseResult:
        """
