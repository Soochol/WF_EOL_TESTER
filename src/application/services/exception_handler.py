...
from domain.exceptions.hardware_exceptions import (
    HardwareException, HardwareNotReadyException, HardwareConnectionException,
    UnsafeOperationException, HardwareTimeoutException, HardwareLimitExceededException
)
from domain.exceptions.test_exceptions import (
    TestExecutionException, InvalidTestStateException, TestSequenceException,
    MeasurementValidationException, TestTimeoutException, TestResourceException
)
from domain.exceptions.configuration_exceptions import (
    ...
    ConfigurationConflictException, ConfigurationSecurityException
)
...
class ExceptionHandler:
    ...
    def classify_exception(self, exception: Exception) -> ExceptionClassification:
        ...
        for rule_type, classification in self._classification_rules.items():
            if isinstance(exception, rule_type):
                logger.debug(f"Classified exception {exception_type.__name__} as {classification.category.value} (inherited from {rule_type.__name__})")
                return classification
