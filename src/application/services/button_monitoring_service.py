...
import asyncio
from typing import TYPE_CHECKING, Callable, Optional, Union, Awaitable, Dict, List
...
class DIOMonitoringService:
    ...
    def __init__(
        ...
        emergency_stop_callback: Optional[Union[Callable[[], None], Callable[[], Awaitable[None]]]] = None,
    ):
        """
        ...
        logger.info(f"🔧 DIO_INIT: Channel configuration built - {len(self._channel_config)} channels configured")

        # Safety parameters
        self._debounce_time = 2.0  # seconds
        self._polling_interval = 0.1  # seconds
        logger.info(f"🔧 DIO_INIT: Safety parameters set - debounce: {self._debounce_time}s, polling: {self._polling_interval}s")
        ...
        for channel, config in self._channel_config.items():
            ...
            logger.info(f"🔧 DIO_INIT: Channel {channel} -> {pin.name} "
    ...
    async def start_monitoring(self) -> None:
        ...
        logger.info(f"🔧 DIO_START: Monitoring parameters - polling interval: {self._polling_interval}s, debounce time: {self._debounce_time}s")
    ...
    def _build_channel_config(self) -> Dict[int, Dict[str, any]]:
        ...
        return {
            ...
            self.chain_sensor_pin.pin_number: {
                'pin': self.chain_sensor_pin,
                'action': None
            },
            self.door_sensor_pin.pin_number: {
                'pin': self.door_sensor_pin,
                'action': None
            },
        }
    ...
    def _detect_channel_edges(self, current_raw_states: List[bool], previous_raw_states: List[bool]) -> List[int]:
        """
        ...
        for channel, config in self._channel_config.items():
            ...
            previous_raw = previous_raw_states[channel] if channel < len(previous_raw_states) else False
            ...
            if edge_detected:
                ...
                logger.debug(f"Edge detected on {pin.name} (channel {channel}): {pin.edge_type} edge, contact_type={pin.contact_type}")
    ...
    async def _process_button_cycle(self) -> None:
        ...
        if self._first_read:
            logger.info(f"🔧 DIO_CYCLE: First read completed - {len(current_raw_states)} channels read")
        
        # Get previous states for edge detection (use empty list for first read)
        previous_raw_states = [] if self._first_read else self._previous_states.copy()
        ...
        if edge_channels:
            ...
            for channel in edge_channels:
                ...
                pin = config['pin']
                logger.info(f"🎯 DIO_CYCLE: Channel {channel} ({pin.name}) - {pin.edge_type} edge detected")
        ...
        for channel in edge_channels:
            ...
            if channel == self.emergency_stop_pin.pin_number:
                ...
                logger.critical(f"🚨 DIO_CYCLE: Emergency stop action triggered on channel {channel}")
                await action()
                ...
                logger.info(f"🎯 DIO_CYCLE: {button_name.title()} button pressed (channel {channel}) - checking dual press conditions")
                await self._handle_operator_button_press(channel, current_raw_states)
                
        # Store current states as previous for next cycle
        self._previous_states = current_raw_states.copy()
        self._first_read = False
        
        # Apply polling interval
        await asyncio.sleep(self._polling_interval)

    async def _handle_operator_button_press(self, pressed_channel: int, current_raw_states: List[bool]) -> None:
        """
        ...
        logger.info(f"🎯 DIO_BUTTON: Button states - Left (ch{left_ch}): raw={left_raw}, pressed={left_pressed}")
        logger.info(f"🎯 DIO_BUTTON: Button states - Right (ch{right_ch}): raw={right_raw}, pressed={right_pressed}")
        
        if left_pressed and right_pressed:
            # Both buttons are currently pressed - check safety conditions
            logger.info("🎯 DIO_BUTTON: ✅ DUAL BUTTON PRESS DETECTED! Checking safety conditions...")
            ...
            if clamp_ok and chain_ok and door_ok:
                logger.info("✅ DIO_SAFETY: All safety sensors satisfied! Proceeding with callback execution...")
            ...
            logger.debug(f"Single button press detected ({pin_name}) - both buttons required for operation")
    ...
    async def _handle_button_press(self) -> None:
        ...
        logger.info(f"✅ DIO_CALLBACK: Button debounce period completed ({self._debounce_time}s) - ready for next press")
    ...
    async def _execute_callback(self) -> None:
        ...
        try:
            ...
            import traceback
            logger.debug(f"❌ CALLBACK_EXEC: Callback execution traceback: {traceback.format_exc()}")
    ...
    async def get_button_states(self) -> dict:
        ...
        try:
            ...
            emergency_stop_state = await self.digital_io.read_input(self.emergency_stop_pin.pin_number)
        ...
        if hasattr(self, 'emergency_stop_callback') and self.emergency_stop_callback:
            ...
            try:
                if asyncio.iscoroutinefunction(self.emergency_stop_callback):
                    logger.critical("🚨 EMERGENCY_CALLBACK: Executing async emergency stop callback...")
                    await self.emergency_stop_callback()
                else:
                    logger.critical("🚨 EMERGENCY_CALLBACK: Executing sync emergency stop callback...")
                    self.emergency_stop_callback()
                logger.critical("✅ EMERGENCY_CALLBACK: Emergency stop callback executed successfully")
            except Exception as e:
                logger.error(f"❌ EMERGENCY_CALLBACK: Error executing emergency stop callback: {e}")
                import traceback
                logger.debug(f"❌ EMERGENCY_CALLBACK: Emergency callback traceback: {traceback.format_exc()}")
        else:
            logger.warning("⚠️ EMERGENCY_CALLBACK: No emergency stop callback configured - only logging emergency stop")
    ...
    async def get_detailed_status(self) -> dict:
        ...
        status = {
            ...
            "emergency_callback_configured": hasattr(self, 'emergency_stop_callback') and self.emergency_stop_callback is not None,
            "debounce_time": self._debounce_time,
            "polling_interval": self._polling_interval,
        ...
        try:
            ...
            if status["digital_io_connected"]:
                ...
                for channel, config in self._channel_config.items():
                    pin = config['pin']
                    raw_state = current_raw_states[channel] if channel < len(current_raw_states) else False
                ...
                left_ch = self.left_button_pin.pin_number
                right_ch = self.right_button_pin.pin_number
                left_pressed = not current_raw_states[left_ch] if left_ch < len(current_raw_states) else False
                right_pressed = not current_raw_states[right_ch] if right_ch < len(current_raw_states) else False
                ...
                status["ready_for_callback"] = (
                    left_pressed and right_pressed and 
                    clamp_ok and chain_ok and door_ok and
    ...
    async def print_verification_report(self) -> None:
        ...
        if "channel_config" in status:
            ...
            for channel, config in status["channel_config"].items():
                logger.info(f"  - Channel {channel}: {config['pin_name']} "
