...
class ConfigurationService:
    ...
    def save_hardware_config(self, config: HardwareConfiguration, file_path: str) -> None:
        ...
        try:
            ...
            full_config = {
                ...
                "repository": {"type": "json", "data_dir": "data/tests", "auto_save": True},
                "application": {"test_timeout": 300, "measurement_precision": 3, "auto_connect": True},
            }
    ...
    def get_default_hardware_config(self) -> HardwareConfiguration:
        ...
        loadcell_config = LoadCellConfig(model="MOCK", port="MOCK", baudrate=9600, timeout=1.0, indicator_id=1)

        mcu_config = MCUConfig(model="MOCK", port="MOCK", baudrate=115200, timeout=2.0)

        power_config = PowerConfig(model="MOCK", host="localhost", port=8080, timeout=5.0, channel=1)

        digital_input_config = DigitalInputConfig(model="MOCK", board_no=0, input_count=32, debounce_time=0.01)

        return HardwareConfiguration(
            robot=robot_config, loadcell=loadcell_config, mcu=mcu_config, power=power_config, digital_input=digital_input_config
        )
    ...
    def get_production_hardware_config(self) -> HardwareConfiguration:
        ...
        loadcell_config = LoadCellConfig(model="BS205", port="COM1", baudrate=9600, timeout=1.0, indicator_id=1)

        mcu_config = MCUConfig(model="LMA", port="COM2", baudrate=9600, timeout=5.0)

        power_config = PowerConfig(model="ODA", host="192.168.1.10", port=8080, timeout=5.0, channel=1)

        digital_input_config = DigitalInputConfig(model="AJINEXTEK", board_no=0, input_count=8, debounce_time=0.01)
