...
class MockRobot(RobotService):
    ...
    def __init__(
        ...
        self._axis_velocities = [
            default_velocity
        ] * axis_count

        logger.info(
            f"MockRobotAdapter initialized with {axis_count} axes"
        )
    ...
    async def connect(
        ...
        """
        logger.info(
            f"Connecting to Mock Robot (IRQ: {robot_config.irq_no}, Axes: {robot_config.axis_count})..."
        )
        ...
        if success:
            ...
            raise HardwareConnectionError(
                "mock_robot", "Simulated connection failure"
            )
    ...
    async def initialize_axes(self) -> None:
        ...
        if not self._is_connected:
            raise HardwareConnectionError(
                "mock_robot", "Robot is not connected"
            )
        ...
        try:
            ...
            logger.info(
                "Mock axis initialization completed successfully"
            )

        except Exception as e:
            logger.error(
                f"Mock axis initialization failed: {e}"
            )
            self._motion_status = MotionStatus.ERROR
            raise HardwareOperationError(
                "mock_robot", "initialize_axes", str(e)
            ) from e
    ...
    async def move_to_position(
        ...
        try:
            logger.info(
                f"Mock robot moving axis {axis} to position {position} at {vel} mm/s"
            )
            self._motion_status = MotionStatus.MOVING

            # 이동 시간 계산
            distance = abs(
                position - self._current_positions[axis]
            )
            ...
            for step in range(steps):
                ...
                self._current_positions[axis] = (
                    start_position
                    + (position - start_position) * progress
                )
            ...
            logger.error(
                f"Mock axis {axis} move failed: {e}"
            )
            self._motion_status = MotionStatus.ERROR
            raise HardwareOperationError(
                "mock_robot", "move_to_position", str(e)
            ) from e
    ...
    async def move_relative_single(
        ...
        # 목표 위치 계산
        target_position = (
            self._current_positions[axis] + distance
        )

        # 절대 이동으로 구현
        await self.move_to_position(
            axis, target_position, velocity
        )
    ...
    async def move_absolute(
        ...
        try:
            ...
            self._motion_status = MotionStatus.IDLE
            logger.info(
                f"Mock robot axis {axis} moved to position {position}mm"
            )

        except Exception as e:
            logger.error(
                f"Failed to move mock robot axis {axis} to position {position}mm: {e}"
            )
            self._motion_status = MotionStatus.ERROR
            raise HardwareOperationError(
                "mock_robot", "move_absolute", str(e)
            ) from e
    ...
    async def move_absolute_multi(
        ...
        try:
            logger.info(
                f"Mock robot moving to positions: {positions} at {vel} mm/s"
            )
            self._motion_status = MotionStatus.MOVING
            ...
            max_distance = max(
                abs(pos - current)
                for pos, current in zip(
                    positions, self._current_positions
                )
            )
            motion_time = (
                max_distance / vel if vel > 0 else 0.1
            )

            # 실제 이동 시뮬레이션
            start_positions = self._current_positions.copy()
            steps = max(
                int(motion_time / 0.1), 1
            )  # 0.1초마다 위치 업데이트
    ...
    async def move_relative_multi(
        ...
        target_positions = [
            current + distance
            for current, distance in zip(
                self._current_positions, distances
            )
        ]

        # 절대 이동으로 구현
        return await self.move_absolute_multi(
            target_positions, velocity
        )
    ...
    async def stop_motion(
        ...
        try:
            ...
            self._motion_status = MotionStatus.IDLE
            logger.info(
                f"Mock robot axis {axis} motion stopped"
            )

        except Exception as e:
            logger.error(
                f"Failed to stop mock robot axis {axis}: {e}"
            )
            raise HardwareOperationError(
                "mock_robot", "stop_motion", str(e)
            ) from e
    ...
    async def emergency_stop(self) -> None:
        ...
        try:
            ...
            raise HardwareOperationError(
                "mock_robot", "emergency_stop", str(e)
            ) from e
    ...
    async def is_moving(
        ...
        if axis is not None and (
            axis < 0 or axis >= self._axis_count
        ):
            return False
    ...
    async def set_velocity(
        ...
        logger.info(
            f"Mock robot axis {axis} velocity set to {velocity} mm/s"
        )
    ...
    def set_position_directly(
        ...
        if len(positions) == self._axis_count:
            ...
            logger.debug(
                f"Mock robot positions set directly to: {positions}"
            )
    ...
    def simulate_error(
        ...
        logger.debug(
            f"Mock robot error simulated: {error_type}"
        )
