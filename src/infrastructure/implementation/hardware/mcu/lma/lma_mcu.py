...
class LMAMCU(MCUService):
    ...
    def _send_packet_sync(
        ...
        try:
            ...
            logger.info(f"\033[94mPC -> MCU:\033[0m \033[95m{packet_hex} ({description})\033[0m")
            ...
            while time.time() - start_time < actual_timeout:
                if self.serial_conn and self.serial_conn.in_waiting > 0:
                    ...
                    logger.debug(
                        f"\033[92mPC <- MCU:\033[0m \033[96m{chunk_hex} (total: {len(response_data)} bytes) @ +{elapsed_ms:.1f}ms\033[0m"
                    )
                    ...
                    if response_data.endswith(b"\xfe\xfe") and len(response_data) >= 6:
                        ...
                        if valid_packet:
                            ...
                            logger.info(f"\033[92mPC <- MCU:\033[0m \033[96m{response_hex} (+{response_time:.1f}ms)\033[0m")
    ...
    async def _wait_for_additional_response(
        ...
        while time.time() - start_time < timeout:
            ...
            if (
                ...
                try:
                    ...
                    if self.serial_conn:
                        ...
                        logger.debug(
                            f"\033[94mPC -> MCU:\033[0m \033[95m{temp_packet_hex} (Temperature request @ +{elapsed:.1f}s)\033[0m"
                        )
            ...
            if self.serial_conn and self.serial_conn.in_waiting > 0:
                ...
                if not quiet:
                    logger.debug(f"\033[92mPC <- MCU:\033[0m \033[96m{chunk_hex} @ +{elapsed_ms:.1f}ms\033[0m")
                ...
                while response_data:
                    ...
                    if valid_packet[2] == 0x07:  # Temperature response
                        logger.debug(
                            f"\033[92mPC <- MCU:\033[0m \033[96m{valid_packet.hex().upper()} ({packet_type}) @ +{elapsed_ms:.1f}ms\033[0m"
                        )
                    else:
                        logger.info(
                            f"\033[92mPC <- MCU:\033[0m \033[96m{valid_packet.hex().upper()} ({packet_type}) @ +{elapsed_ms:.1f}ms\033[0m"
        ...
        try:
            ...
            while time.time() - start_time < boot_timeout:
                if self.serial_conn and self.serial_conn.in_waiting > 0:
                    ...
                    logger.debug(f"\033[92mPC <- MCU:\033[0m \033[96m{chunk_hex} (boot data) @ +{elapsed_ms:.1f}ms\033[0m")
                    ...
                    if response_data.endswith(b"\xfe\xfe") and len(response_data) >= 6:
                        ...
                        if valid_packet:
                            ...
                            if len(valid_packet) >= 6 and valid_packet[2] == 0x00:
                                response_hex = valid_packet.hex().upper()
                                logger.info(f"\033[92mPC <- MCU:\033[0m \033[96m{response_hex} (boot complete confirmed)\033[0m")
                                ...
                                logger.info(
                                    f"\033[92mPC <- MCU:\033[0m \033[96m{response_hex} (status: 0x{status_code:02X}, not boot complete)\033[0m"
                                )
        ...
        for attempt in range(max_retries):
            try:
                ...
                if self.serial_conn:
                    ...
                    raise HardwareConnectionError("fast_lma_mcu", "Serial connection not available")
                logger.info(f"\033[94mPC -> MCU:\033[0m \033[95m{packet} (CMD_SET_OPERATING_TEMP ({target_temp}°C))\033[0m")
        ...
        for attempt in range(max_retries):
            try:
                ...
                if self.serial_conn:
                    ...
                    raise HardwareConnectionError("fast_lma_mcu", "Serial connection not available")
                logger.info(f"\033[94mPC -> MCU:\033[0m \033[95m{packet} (CMD_SET_COOLING_TEMP ({target_temp}°C))\033[0m")
    ...
    async def set_test_mode(self, mode: TestMode) -> None:
        ...
        try:
            ...
            if self.serial_conn:
                ...
                raise HardwareConnectionError("fast_lma_mcu", "Serial connection not available")
            logger.info(f"\033[94mPC -> MCU:\033[0m \033[95m{packet} (CMD_ENTER_TEST_MODE (mode {mode_value}))\033[0m")
    ...
    async def set_upper_temperature(self, upper_temp: float) -> None:
        ...
        try:
            ...
            if self.serial_conn:
                ...
                raise HardwareConnectionError("fast_lma_mcu", "Serial connection not available")
            logger.info(f"\033[94mPC -> MCU:\033[0m \033[95m{packet} (CMD_SET_UPPER_TEMP ({upper_temp}°C))\033[0m")
    ...
    async def set_fan_speed(self, fan_level: int) -> None:
        ...
        try:
            ...
            if self.serial_conn:
                ...
                raise HardwareConnectionError("fast_lma_mcu", "Serial connection not available")
            logger.info(f"\033[94mPC -> MCU:\033[0m \033[95m{packet} (CMD_SET_FAN_SPEED (level {fan_level}))\033[0m")
    ...
    async def start_standby_heating(
        ...
        for attempt in range(max_retries):
            try:
                ...
                logger.info(f"\033[94mPC -> MCU:\033[0m \033[95m{packet} (Heating: {standby_temp}°C → {operating_temp}°C)\033[0m")
    ...
    async def start_standby_cooling(self, max_retries: int = 3) -> None:
        ...
        for attempt in range(max_retries):
            try:
                ...
                command_sent_time = time.perf_counter()
                logger.info(f"\033[94mPC -> MCU:\033[0m \033[95m{packet} (Cooling: {from_temp}°C → {to_temp}°C)\033[0m")
