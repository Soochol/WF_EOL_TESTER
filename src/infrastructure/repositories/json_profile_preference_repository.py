"""
JSON Profile Preference Repository
...
class JsonProfilePreferenceRepository(ProfilePreferenceRepository):
    """
    ...
    async def save_last_used_profile(self, profile_name: str) -> None:
        ...
        try:
            ...
            old_profile = preferences.get("last_used_profile")
            ...
            if old_profile != profile_name:
                logger.debug(f"Last used profile updated: '{old_profile}' → '{profile_name}'")
    ...
    async def load_last_used_profile(self) -> Optional[str]:
        ...
        try:
            preferences = await self._load_preferences()
            profile_name = preferences.get("last_used_profile")
    ...
    async def _save_preferences(self, preferences: Dict[str, Any]) -> None:
        ...
        preferences["_metadata"] = {
            "version": "1.0",
            "created_by": "JsonProfilePreferenceRepository",
    ...
    def set_max_history_entries(self, max_entries: int) -> None:
        """
        ...
        if max_entries > 0:
            ...
            logger.debug(f"Max history entries set to: {max_entries}")
        else:
            logger.warning(f"Invalid max history entries: {max_entries}")
