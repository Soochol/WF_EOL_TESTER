...
from application.interfaces.test_result_repository import TestResultRepository
...
class JsonTestResultRepository(TestResultRepository):
    ...
    def __init__(self, data_dir: str = "data/tests", auto_save: bool = True):
        ...
        logger.info(f"JsonTestResultRepository initialized at {self._data_dir}")
    ...
    async def save(self, test: EOLTest) -> EOLTest:
        ...
        if isinstance(test, dict):
            logger.warning("Received dictionary instead of EOLTest object, attempting to convert")
            try:
                test = EOLTest.from_dict(test)
            except Exception as e:
                logger.error(f"Failed to convert dictionary to EOLTest: {e}")
                raise ValueError(f"Expected EOLTest object but received dictionary: {e}")
        
        if not hasattr(test, 'test_id'):
            logger.error(f"Invalid test object: {type(test)}, {test}")
            raise ValueError(f"Invalid test object, missing test_id attribute: {type(test)}")
        ...
        logger.debug(f"Test result {test_id} saved to repository")
        return test
    ...
    async def update(self, test: EOLTest) -> EOLTest:
        ...
        Returns:
            수정된 테스트 결과
        """
        # Enhanced validation for update as well
        if isinstance(test, dict):
            logger.warning("Received dictionary instead of EOLTest object in update, attempting to convert")
            try:
                test = EOLTest.from_dict(test)
            except Exception as e:
                logger.error(f"Failed to convert dictionary to EOLTest in update: {e}")
                raise ValueError(f"Expected EOLTest object but received dictionary in update: {e}")
    ...
    async def find_by_dut_id(self, dut_id: str) -> List[EOLTest]:
        ...
        logger.debug(f"Found {len(tests)} test results for DUT {dut_id}")
        return tests
        ...
        try:
            ...
            if not file_path.exists() and test_id not in self._tests_cache:
                raise ConfigurationNotFoundError(
                    f"Test result {test_id} not found",
            ...
            logger.debug(f"Test result {test_id} deleted from repository")
            
        except ConfigurationNotFoundError:
            raise
        except Exception as e:
            logger.error(f"Failed to delete test result {test_id}: {e}")
            raise RepositoryAccessError(
                f"Failed to delete test result {test_id}: {str(e)}",
    ...
    async def _test_to_dict(self, test: EOLTest) -> Dict[str, Any]:
        """테스트 엔티티를 딕셔너리로 변환"""
        try:
            # Check if test is already a dictionary (defensive programming)
            if isinstance(test, dict):
                logger.warning("Test is already a dictionary, returning as-is")
                return test
            ...
            logger.debug(f"Test type: {type(test)}, Test: {test}")
            raise
    ...
    async def _save_to_file(self, test_id: str, test_dict: Dict[str, Any]) -> None:
        ...
        try:
            ...
            logger.debug(f"Test result {test_id} saved to file {file_path}")
            
        except Exception as e:
            logger.error(f"Failed to save test result {test_id} to file: {e}")
            raise
    ...
    async def _load_from_file(self, test_id: str) -> Optional[Dict[str, Any]]:
        ...
        try:
            ...
            logger.debug(f"Test result {test_id} loaded from file {file_path}")
            return test_dict
            
        except Exception as e:
            logger.error(f"Failed to load test result {test_id} from file: {e}")
            return None
    
    async def _load_all_tests(self) -> None:
        """모든 테스트 결과 파일을 캐시로 로드"""
                ...
                try:
                    ...
                    if test_date < cutoff_date:
                        try:
                            ...
                            logger.warning(f"Failed to delete old test result {test_id}: {e}")
                except Exception as e:
                    logger.warning(f"Failed to parse date for test result {test_id}: {e}")
