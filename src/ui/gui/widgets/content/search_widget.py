...
class SearchWidget(QWidget):
    ...
    def setup_ui(self) -> None:
        ...
        self.filters_panel.delete_requested.connect(self._on_delete_clicked)
    ...
    def _on_selection_changed(self, selected_count: int) -> None:
        ...
        # Update Delete button in filters panel
        if self.filters_panel:
            self.filters_panel.set_delete_button_enabled(selected_count > 0)
    ...
    async def _delete_tests(self, test_ids: List[str]) -> None:
        ...
        try:
            ...
            if failed:
                ...
                error_details = "\n".join(
                    [f"- {tid}: {errors.get(tid, 'Unknown error')}" for tid in failed]
                )
                QMessageBox.warning(
                    self,
                    "Partial Deletion",
