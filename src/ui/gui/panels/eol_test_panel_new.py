...
class EOLTestPanel(QWidget):
    ...
    def update_ui_for_test_start(self) -> None:
        ...
        self.ui.summaryLabel.setStyleSheet("color: #3498DB; padding: 4px; font-weight: bold; font-size: 11px;")
    ...
    def reset_test_ui(self) -> None:
        ...
        self.ui.summaryLabel.setStyleSheet("color: #7F8C8D; padding: 4px; font-weight: bold; font-size: 11px;")
        ...
        try:
            ...
            if hasattr(result, 'measurements') and result.measurements:
                ...
                for row, measurement in enumerate(measurements):
                    ...
                    # Value
                    value_str = f"{measurement.value:.2f}" if isinstance(measurement.value, (int, float)) else str(measurement.value)
    ...
    def on_test_completed(self, result) -> None:
        ...
        try:
            ...
            # Update summary
            self.ui.summaryLabel.setText(f"Test {status}: {len(getattr(result, 'measurements', []))} measurements")
            if status == "PASSED":
                self.ui.summaryLabel.setStyleSheet("color: #27AE60; padding: 4px; font-weight: bold; font-size: 11px;")
            else:
                self.ui.summaryLabel.setStyleSheet("color: #E74C3C; padding: 4px; font-weight: bold; font-size: 11px;")
