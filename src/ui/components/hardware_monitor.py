...
class HardwareMonitorComponent:
    ...
    def __init__(self, refresh_interval: int = 2000):
        ...
        self._alerts: deque[HardwareAlert] = deque(
            maxlen=100
    ...
    def register_device(
        ...
        if not device_id or len(device_id) > 64:
            raise ValueError(
                "Invalid device_id: must be 1-64 characters"
            )
        ...
        for device_id, device in self._devices.items():
            try:
                ...
                self._create_alert(
                    ...
                    f"Device connection lost: {str(e)}",
                )
            except TimeoutError as e:
                self._create_alert(
                    device_id,
                    AlertSeverity.WARNING,
                    f"Device response timeout: {str(e)}",
                )
    ...
    async def _update_device_status(
        self, device_id: str, device: MonitoredDevice
    ) -> None:
    ...
    def _calculate_health_score(
        self, device_id: str, metrics: MonitoringMetrics
    ) -> float:
        ...
        time_since_comm = (
            datetime.now() - metrics.last_communication
        )
    ...
    def _create_alert(
        ...
        # Note: deque automatically limits to maxlen=100, no manual cleanup needed
    ...
    def get_device_grid_data(self) -> List[Dict[str, Any]]:
        ...
        for device_id, device in self._devices.items():
            ...
            recent_alerts_list = list(self._alerts)
            recent_alerts = [
                alert
                for alert in recent_alerts_list[-10:]
            ...
            grid_data.append(
                {
                    ...
                    "status_color": self._get_status_color(
                        device.status, metrics.health_score
                    ),
                }
            )
    ...
    def _get_status_color(
        self, status: HardwareStatus, health_score: float
    ) -> str:
    ...
    def get_alert_summary(self) -> Dict[str, Any]:
        ...
        return {
            ...
            "last_alert": (
                active_alerts[-1].timestamp.isoformat()
                if active_alerts
                else None
            ),
        }
...
"""
        ...
        if not devices:
            ...
            for device in devices[
                ...
                health_icon = self._get_health_icon(
                    device["health_score"]
                )
...
class WebSocketManager:
    ...
    async def broadcast_update(
        ...
        print(
            f"WebSocket broadcast: {json.dumps(message, default=str)}"
        )
    ...
    def handle_monitoring_event(
        ...
        asyncio.create_task(
            self.broadcast_update(event_type, data)
        )
