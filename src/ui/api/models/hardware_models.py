...
class HardwareStatusResponse(BaseModel):
    ...
    def from_status_dict(cls, status: Dict[str, bool]) -> "HardwareStatusResponse":
        ...
        overall_status = "connected" if all_connected else "partial" if any(status.values()) else "disconnected"
...
class RobotControlRequest(BaseModel):
    ...
    operation: str = Field(..., description="Operation to perform: 'home', 'move', 'enable', 'disable'")
...
class PowerControlRequest(BaseModel):
    ...
    operation: str = Field(..., description="Operation: 'enable', 'disable', 'set_voltage', 'set_current'")
...
class MCUControlRequest(BaseModel):
    ...
    operation: str = Field(..., description="Operation: 'set_temperature', 'set_fan_speed', 'test_mode'")
...
class DigitalIORequest(BaseModel):
    ...
    operation: str = Field(..., description="Operation: 'read_input', 'write_output', 'read_all_inputs'")
...
class HardwareConnectionRequest(BaseModel):
    ...
    hardware_types: Optional[List[str]] = Field(None, description="Specific hardware to connect/disconnect")
