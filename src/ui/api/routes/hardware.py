...
async def get_robot_status(container: DIContainer = Depends(get_container)):
    """Get robot status"""
    try:
        robot_service: RobotService = container.robot_service()
...
async def get_power_status(container: DIContainer = Depends(get_container)):
    ...
    try:
        power_service: PowerService = container.power_service()
...
async def connect_power(
    ...
    try:
        ...
        try:
            status = await power_service.get_status()
            device_identity = status.get('device_identity')
        except:
            pass
...
async def set_power_current_limit(
    ...
    try:
        ...
        if current_limit is None:
            raise HTTPException(
                status_code=status.HTTP_400_BAD_REQUEST,
                detail="Current limit is required"
            )
...
async def set_mcu_upper_temperature(
    ...
    try:
        ...
        if upper_temperature is None:
            raise HTTPException(
                status_code=status.HTTP_400_BAD_REQUEST,
                detail="Upper temperature is required"
            )
...
async def set_mcu_test_mode(
    ...
    try:
        ...
        return {
            ...
            "message": message,
            "data": {
                "test_mode": mode_enum.value,
                "test_mode_name": mode_enum.name
            }
        }
...
async def start_mcu_standby_heating(
    ...
    try:
        ...
        message = f"MCU standby heating started: {operating_temp}°C operating, {standby_temp}°C standby"
    ...
    try:
        ...
        return {
            ...
            "data": {
                ...
                "timestamp": force_value.timestamp.isoformat() if force_value.timestamp else None
            }
        }
...
async def connect_digital_io(request: Dict = None, container: DIContainer = Depends(get_container)):
    """Connect to Digital I/O hardware"""
    try:
        digital_io_service: DigitalIOService = container.digital_io_service()
        ...
        return {
            "success": True,
            "message": f"Digital I/O connected successfully",
            "data": {
                "irq_no": irq_no,
...
async def get_digital_io_status(container: DIContainer = Depends(get_container)):
    """Get Digital I/O hardware status"""
    try:
        digital_io_service: DigitalIOService = container.digital_io_service()
...
async def write_digital_output(request: Dict, container: DIContainer = Depends(get_container)):
...
async def read_multiple_digital_inputs(request: Dict, container: DIContainer = Depends(get_container)):
    ...
    try:
        ...
        for channel in channels:
            if not isinstance(channel, int) or channel < 0:
                return {"success": False, "error": f"Invalid channel: {channel}. All channels must be non-negative integers"}
...
async def write_multiple_digital_outputs(request: Dict, container: DIContainer = Depends(get_container)):
    ...
    try:
        ...
        if not isinstance(pin_values, dict):
            return {"success": False, "error": "pin_values must be a dictionary mapping channel numbers to boolean values"}
            ...
            try:
                ...
                if not isinstance(level, bool):
                    if isinstance(level, str):
                        ...
                        return {"success": False, "error": f"Invalid level for channel {channel}: must be boolean"}
                
                validated_pin_values[channel] = level
                
            except ValueError:
                return {"success": False, "error": f"Invalid channel: {channel_str}. Must be an integer"}
        ...
        if success:
            ...
            return {
                "success": True,
                "message": f"Multiple digital outputs written successfully ({len(validated_pin_values)} channels)",
                "data": {
                    "outputs": outputs_data,
