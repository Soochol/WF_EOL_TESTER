...
async def websocket_digital_input(websocket: WebSocket):
    ...
    if "digital-input" not in background_tasks:
        background_tasks["digital-input"] = asyncio.create_task(
            digital_input_monitor()
        )
        ...
        while True:
            ...
            try:
                ...
                if message.get("type") == "subscription":
                    ...
                    await manager.send_personal_message(websocket, response.dict())
...
async def websocket_system_status(websocket: WebSocket):
    ...
    if "system-status" not in background_tasks:
        background_tasks["system-status"] = asyncio.create_task(
                ...
                if await digital_io_service.is_connected():
                    ...
                    for channel in range(32):
                        try:
                            ...
                            # Check for changes
                            if channel not in last_digital_inputs or last_digital_inputs[channel] != value:
                                changed_channels.append(channel)
                                
                        except Exception as e:
                            # If read fails, default to False
                            current_inputs[channel] = False
                            if channel not in last_digital_inputs or last_digital_inputs[channel] != False:
                                changed_channels.append(channel)
                    ...
                    if changed_channels:
                        ...
                        await manager.broadcast_to_channel("digital-input", message.dict())
        ...
        while active_connections["system-status"]:
            try:
                ...
                if current_status != last_status:
                    ...
                    await manager.broadcast_to_channel("system-status", message.dict())
...
async def broadcast_test_log(
    ...
    await manager.broadcast_to_channel("test-logs", log_message.dict())
...
async def broadcast_test_progress(
    ...
    await manager.broadcast_to_channel("test-logs", progress_message.dict())
...
async def broadcast_hardware_event(
    ...
    await manager.broadcast_to_channel("system-status", event_message.dict())
