...
class DigitalIOController(HardwareController):
        ...
        try:
            ...
            if is_connected:
                ...
                try:
                    ...
                    self.formatter.console.print(f"  Hardware Type: Digital I/O")
            ...
            self.formatter.print_message(
                f"Failed to get Digital I/O status: {str(e)}", 
                message_type="error"
            )
    ...
    async def disconnect(self) -> bool:
        ...
        return await self._show_progress_with_message(
            ...
            self.digital_io_service.disconnect,
    ...
    async def show_control_menu(self) -> Optional[str]:
        ...
        menu_options = {
            "1": "Show All Output States",
            "2": "Toggle Output Channel",
            "3": "Reset All Outputs",
    ...
    async def _show_all_output_states(self) -> None:
        ...
        try:
            ...
            if not self._current_outputs:
                self.formatter.print_message(
                    "No output channels available or hardware not connected",
                    message_type="warning"
                )
                return
            ...
            self.formatter.print_message(
                f"Failed to read output states: {str(e)}", 
                message_type="error"
            )
    ...
    async def _toggle_output_interactive(self) -> None:
        ...
        try:
            ...
            self.formatter.print_message(
                "Enter channel number to toggle (or 'cancel' to abort):", 
                message_type="info"
            )
            ...
            while True:
                try:
                    ...
                    if success:
                        ...
                        self.formatter.print_message(
                            f"Channel {channel} toggled → {state_str}",
                    ...
                    self.formatter.print_message(
                        f"Toggle error: {str(e)}", 
                        message_type="error"
                    )
                    
        except Exception as e:
            self.formatter.print_message(
                f"Interactive toggle error: {str(e)}", 
                message_type="error"
            )
    ...
    async def _toggle_connection(self) -> None:
        ...
        try:
            ...
            self.formatter.print_message(
                f"Connection toggle error: {str(e)}", 
                message_type="error"
            )
